    ...
    public boolean insertInto(String table, String[] variables) {
        try {
            ...
            conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).execute("insert into `" + table + "` " +
                    "values ('" + values + "');");
            return true;
...
//                }
    ...
    public void logIn(Account account, String username, String password) {
        ...
        try {
            ...
            if (!checkPassword.getString(1).equals(password)) {
                System.err.println("Password is incorrect");
                return;
            }

            //get local type from db
            ResultSet getType = inquire("select type from `account` where username = '" + username + "'");
            getType.next();

            //set static type
            //as the user is logged in,  it makes sense to be able to access their type statically.
            account.setType(getType.getString(1));
